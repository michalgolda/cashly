...
class ExpenseRepository(Repository):
	...
	def get_one_by(self, **kwargs) -> typing.Union[models.Expense, None]:
		with self._db.session_factory() as session:
	...
	def get_all_by(self, **kwargs) -> typing.List[models.Expense]:
		with self._db.session_factory() as session:
			expenses = session.query(models.Expense).filter_by(**kwargs).all()
...
class ExpenseCategoryRepository(Repository):
	...
	def get_one_by(self, **kwargs) -> typing.Union[models.ExpenseCategory, None]:
		with self._db.session_factory() as session:
			expense_category = session.query(models.ExpenseCategory) \
				.filter_by(**kwargs).first()
	...
	def get_all_by(self, **kwargs) -> typing.List[models.ExpenseCategory]:
		with self._db.session_factory() as session:
			expense_categories = session.query(models.ExpenseCategory).filter_by(**kwargs).all()
